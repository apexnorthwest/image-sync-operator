...
RUN set -e ; if lscpu | grep -q x86_64; then \
    curl -L https://github.com/anchore/syft/releases/download/v1.50.0/syft_1.50.0_linux_amd64.deb -o /tmp/syft.deb && dpkg -i /tmp/syft.deb && rm /tmp/syft.deb; \
  ...
  curl -L https://github.com/anchore/syft/releases/download/v1.50.0/syft_1.50.0_linux_arm64.deb -o /tmp/syft.deb && dpkg -i /tmp/syft.deb && rm /tmp/syft.deb; \
